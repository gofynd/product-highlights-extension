...
module.exports = function (fastify, opts, done) {
    ...
    fastify.get('/applications', async function view(req, res) {
        try {
    ...
    // get product list
    fastify.get('/:application_id/products', async function view(req, res) {
        try {
            const platformClient = await fdkExtension.getPlatformClient(req.fdkSession.company_id, req.fdkSession);
    ...
    fastify.post("/:application_id/product/:item_id/highlights", async function view(req, res) {
        try {
            ...
            return res.status(200).send(productHighlightRecord);
    ...
    fastify.get("/:application_id/highlight/list", async function view(req, res) {
        try {
            const { application_id } = req.params;
            const companyId = Number(req.fdkSession.company_id);
    ...
    fastify.get("/:application_id/highlight", async function view(req, res) {
        try {
            const { application_id } = req.params;
            ...
            if (item_code) {
                ...
                return res.status(400).send({"Error": "Invalid Item code or slug in the query param"});
            }
    ...
    fastify.delete("/:application_id/highlight", async function view(req, res) {
        try {
            const { application_id } = req.params;
